...
party_name <- c(DEM="Democratic",
                REP="Republican")
Transaction_type <- c(pro="Advocating election of candidate",
                      con="opposing election of candidate")
```
...
ggplot(election_total, aes(x = entity.type, y = amount/party_entity_total*100, fill=transaction.type, width = 0.7)) + 
  ...
  labs(title = "Distribution of Contributions \nby Party and Entity Type",x="Entity Type", y="Contributions (%)", fill = "Transaction Type") +
...
p2 <- ggplot(election_non_org, aes(x = entity.type, y = party_entity_total, fill=transaction.type)) + 
  geom_bar(stat = "identity", alpha = 0.8) + 
  facet_grid(.~cand_party_affiliation,labeller=as_labeller(party_name)) +
  labs(title = "Distribution of Contributions by Party and Entity Type (Non-ORG)",x="Entity Type", y="Amount of Contributions \n(in USD)", fill="Transaction Type") +
  theme(plot.title=element_text(size= 18, 
                                face="bold", 
                                family="Helvetica", 
                                hjust=0.5,
...
p3 <- ggplot(election_org_only, aes(x = entity.type, y = party_entity_total, fill=transaction.type, width=0.15)) + 
  geom_bar(stat = "identity",alpha=0.8) + 
  facet_grid(.~cand_party_affiliation,labeller=as_labeller(party_name)) +
  labs(title = "Distribution of Contributions by Party and Entity Type (ORG)",x="Entity Type", y="Amount of Contributions \n(in USD)", fill="Transaction Type") +
  theme(plot.title=element_text(size=18, 
                                face="bold", 
                                family="Helvetica", 
                                hjust=0.5,
