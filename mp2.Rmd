...
com.id <- c(com$cmte_id)

con <- contributions %>%
  filter(cmte_id %in% c(com.id))

con2 <- left_join(con, com, by = "cmte_id")
...
election <- full_join(can, con2, by = "cand_id") %>% ## New dataset by joining candidate and contribution dataset
